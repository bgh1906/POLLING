...
//    }
    ...
    public Docket swaggerMainApi() {
        return new Docket(DocumentationType.SWAGGER_2).useDefaultResponseMessages(false)
                .groupName("Main")
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.polling.api.controller"))
                .paths(PathSelectors.ant("/api/**"))
                .build()
    ...
    @Bean
    public Docket swaggerAuthApi() {
        return new Docket(DocumentationType.SWAGGER_2).useDefaultResponseMessages(false)
                .groupName("Auth")
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.polling.common.security.controller"))
                .paths(PathSelectors.ant("/api/**"))
                .build()
                .apiInfo(new ApiInfoBuilder().version("1.0").title("Auth API").build());
    }
